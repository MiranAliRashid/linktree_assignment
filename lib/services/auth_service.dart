...
class AuthService extends ChangeNotifier {
  ...
  void setTheGeneralUser(UserModel theGUser) {
  ...
  Future<bool> fetchUserInfo(String uid) async {
    DocumentSnapshot _userSnap =
        await _firebaseFirestore.collection('users_linktree').doc(uid).get();
    if (_userSnap.exists) {
      //map the data to a general_user data model
      UserModel _generalUser =
          UserModel.fromMap(_userSnap.data() as Map<String, dynamic>);
